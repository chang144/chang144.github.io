import{_ as e,o as n,c as i,e as d}from"./app-2b4e389e.js";const s={},l=d(`<h1 id="使用docker-以及docker-compose部署go程序" tabindex="-1"><a class="header-anchor" href="#使用docker-以及docker-compose部署go程序" aria-hidden="true">#</a> 使用Docker 以及Docker Compose部署Go程序</h1><h2 id="为什么需要docker" tabindex="-1"><a class="header-anchor" href="#为什么需要docker" aria-hidden="true">#</a> 为什么需要docker</h2><blockquote><p>使用docker的主要目标是其容器化。可以为应用程序提供一致的环境，而不依赖它运行的主机</p></blockquote><hr><h3 id="部署示例" tabindex="-1"><a class="header-anchor" href="#部署示例" aria-hidden="true">#</a> 部署示例</h3><h4 id="_1-准备代码" tabindex="-1"><a class="header-anchor" href="#_1-准备代码" aria-hidden="true">#</a> 1.准备代码</h4><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>package main
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>标志位<code>-p</code>来定义端口绑定，由于容器中的应用程序在端口8888上运行，这里绑定的主机端口也是8888。如果要绑定另一个端口，则可以使用 <code>-p $HOST_PORT:8888</code></p><p>到这里就可以测试我们的程序是否工作正常，打开 http://127.0.0.1:8888 查看事先定义的响应内容。</p><hr><h3 id="分阶段构建示例" tabindex="-1"><a class="header-anchor" href="#分阶段构建示例" aria-hidden="true">#</a> 分阶段构建示例</h3><p>Go程序编译之后可得到一个可执行的二进制文件，在最终的镜像中不需要go编译器，也就是说我们只需要一个运行最终二进制文件的容器即可。</p><p>Docker的最佳实践之一是通过仅保留二进制文件来减小镜像大小，为此，我们将使用一种称为多阶段构建的技术</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>FROM golang:alpine AS builder
...
# ENTRYPOINT [&quot;/bubble&quot;, &quot;conf/config.ini&quot;]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第二个地方是在<code>bubble_app</code>下面添加如下命令，使用提前编写的<code>wait-for.sh</code>脚本检测<code>mysql8019:3306</code>正常后再执行后续启动Web应用程序的命令：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>command: sh -c &quot;./wait-for.sh mysql8019:3306 -- ./bubble ./conf/config.ini&quot;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,73),a=[l];function c(r,o){return n(),i("div",null,a)}const u=e(s,[["render",c],["__file","go docker部署.html.vue"]]);export{u as default};
